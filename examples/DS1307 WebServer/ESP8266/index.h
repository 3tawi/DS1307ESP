...
function myFunctionS() {
  var request = new XMLHttpRequest();
  request.open("GET", "changefont", false);
  request.send(null);
  GetArduinoInputs();
}
  ...
  xhttp.onreadystatechange = function() {
    if (this.readyState == 4 && this.status == 200) {
            ...
            lang_font = this.responseXML.getElementsByTagName('analog')[1].childNodes[0].nodeValue;
    }
    document.getElementById("fontmesg").innerHTML = lang_font;
...
.DataCard{
  ...
  <div class="data-input">
   ...
   <td style="width:24%">
    <div class="slidecontainer">
   <input type="number" value="0" name="locatz" id="mylocatz" min="-12" max="+12" class="button-blue" style="width:20%">
    <b class="button-blue" style="width:76%"><span id="TZmesg" onclick="myFunctionTz()"></span></b>
   </div></td>
   <td style="width:64%">
   ...
   <td style="width:10%">
    <b class="button-blue" style="width:96%"><span id="fontmesg" onclick="myFunctionS()"></span></b>
   </td>
  </table>
  </div></div>
  </tr>
  ...
  <div class="data-input">
   ...
   <div class="slidecontainer">
  <form action="/" method="POST">
  <input type="submit" name = "update" value="Update page" class="button-blue">
  </form>
  </div></td>
   <td style="width:22%">
   <div class="slidecontainer">
  <form action="/restesp" method="POST">
  <input type="submit" name = "rerset" value="Restart" class="button-blue">
  </form>
  </div></td>
  </tr>
  </table>
